...
Given a `date`, return an ArrayList of file names that correspond to this date. This list should have the file names in the order that they were added *with the exception of duplicates*. When a duplicate file is passed in, it should *update* the original FileData without changing its position in the ArrayList.   
